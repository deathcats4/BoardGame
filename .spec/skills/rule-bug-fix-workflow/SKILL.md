...
5. **拆原子断言**
   ...
   - 若反馈涉及不能点击、不能打出、资源不足、次数用尽、阶段不允许或目标非法，必须单列“玩家禁止操作反馈”：入口是隐藏、禁用还是可见但会拒绝；拒绝时玩家看到哪一句 toast / 短提示；拒绝后资源、次数、阶段和选中态是否保持不变。
...
- 原始症状与目标对象。
- 录入合同状态和路径。
- bug 判定：是 bug、不是 bug、还是证据不足。不是 bug 时必须说明同源证据为什么能证明当前行为符合规则或产品合同；不能用“如果正常操作能成功”这类抽象条件反驳用户实测。
- 结论：实现 bug、录入合同 bug、二者都有、还是证据不足。
- 原因链：是 bug 时必须用玩家能听懂的话写清 `玩家动作 -> 状态写入 -> 消费者或分支 -> 玩家可见结果或最终权威状态`；缺少任一环节证据时，不能写根因已定位。
