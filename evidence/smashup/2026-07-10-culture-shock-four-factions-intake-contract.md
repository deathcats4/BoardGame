...
- 当前已进入运行时复核阶段：阿南西传说、格林童话、俄罗斯童话各有领域测试与代表性真实入口；古代印加人本地玩法状态记为 `current_scope_closed`，星星上的征兆三条真实生命周期 E2E 已通过。
- 四派系批次状态仍为 `in_progress`，但不再因为星星上的征兆保留古代印加人的对象级残余。旧 intake 文档中的“尚未进入运行时实施”只保留为历史阶段记录，不能覆盖当前代码证据；本轮以各派系 progress 文档、四个领域测试文件和四个 E2E 文件为准。
- 远端资源公开 URL / `HEAD 200` 仍是共享残余，不单独作为规则实现已完成的证明。

## 旧结论失效与当前状态

- 旧结论：本文件早期把四派系全部标成 `pending`，并把“尚未进入运行时实施”作为当前事实。
- 失效原因：后续运行时实现、领域测试和真实入口 E2E 已经改变了四派系的当前事实，旧 intake 阶段状态不能覆盖新 evidence。
- 替代证据或替代入口：以四派系 progress 文档、各派系领域测试、共享合同测试和各派系 E2E 文件为当前运行时入口；古代印加人以当前范围关闭证据为准，其余派系仍按各自残余范围处理。
- 降级后当前状态：本文件继续承担 intake 来源合同和历史阶段记录，不承担四派系玩法完成宣告；批次仍是 `in_progress`，旧的 `pending` 表格只保留历史 intake 快照。

## 同类扩审与残余范围

- 搜索范围：四派系数据定义、能力注册、共享 reducer / reaction queue / base ability 链、领域测试、真实入口 E2E 和资源 manifest；未把历史 intake 表格当作当前运行时事实。
- 命中项：阿南西传说、格林童话、俄罗斯童话已有代表性领域 / 真实入口证据；古代印加人当前本地玩法范围已由 14 个对象的实现、共享回归和真实入口证据替换旧阶段状态。
- 残余扩审范围：其它三个派系的对象级 direct L3/L4、逐派系 evidence 门禁补齐，以及文化冲击 atlas 的 R2 上传和 CDN `HEAD 200`。
