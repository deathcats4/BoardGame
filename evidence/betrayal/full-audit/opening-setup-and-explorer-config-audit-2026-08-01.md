...
- 测试语义对账：角色选择、全员确认、剧本卡确认、正式开始和开局持有物边界均按真实状态转移核对；不是只断言按钮出现或 handler 被调用。
- 旧测试失效检查：历史上允许未选择玩家被默认角色补上的旧语义已被当前门禁替换；当前回归测试明确断言“未选完全部玩家不能开始”，不再把旧测试结果当作有效证据。
- 最终状态断言：测试回到 `preHaunt`、所选角色被正式消费、所有探索者位于 `entrance-hall`、三名探索者背包为空、`turnStartInventoryCardIds` 为空、牌堆数量为 9/22/43、全员预兆总数为 0。
