    ...
    switch (command.type) {
        case SU_COMMANDS.PLAY_MINION: {
            ...
            if (minionResponseWindow && minionResponseWindow.windowType === 'meFirst') {
                ...
                if (!mfCard) return { valid: false, error: '鎵嬬墝涓病鏈夎鍗＄墝' };
                if (mfCard.type !== 'minion') return { valid: false, error: '璇ュ崱鐗屼笉鏄殢浠? };
                const mfDef = getMinionDef(mfCard.defId);
                if (!mfDef) return { valid: false, error: '鍗＄墝瀹氫箟涓嶅瓨鍦? };
                if (!mfDef.beforeScoringPlayable) {
                    return { valid: false, error: '璇ラ殢浠庝笉鑳藉湪鍩哄湴璁″垎鍓嶆墦鍑? };
                }
            ...
            if (fromDiscard) {
                ...
                if (!discardCard || discardCard.type !== 'minion') {
                    return { valid: false, error: '寮冪墝鍫嗕腑娌℃湁璇ラ殢浠? };
                }
            ...
            if (!card) return { valid: false, error: '鎵嬬墝涓病鏈夎鍗＄墝' };
            if (card.type !== 'minion') return { valid: false, error: '璇ュ崱鐗屼笉鏄殢浠? };
        ...
        case SU_COMMANDS.PLAY_ACTION: {
            ...
            if (responseWindow && (responseWindow.windowType === 'meFirst' || responseWindow.windowType === 'afterScoring')) {
                ...
                if (rDef.subtype !== 'special') {
                    ...
                    return { valid: false, error: '鍝嶅簲绐楀彛鍙兘鎵撳嚭鐗规畩琛屽姩鍗? };
                }
                
                // 妫€鏌?specialTiming 鏄惁鍖归厤绐楀彛绫诲瀷
                const cardTiming = rDef.specialTiming ?? 'beforeScoring'; // 榛樿涓?beforeScoring
                ...
                if (rDef.specialNeedsBase) {
                    if (typeof targetBase !== 'number' || !Number.isInteger(targetBase)) {
                        ...
                        return { valid: false, error: '璇ョ壒娈婅鍔ㄥ崱闇€瑕侀€夋嫨涓€涓揪鏍囧熀鍦? };
                    }
                    ...
                    return { valid: false, error: '璇ョ壒娈婅鍔ㄥ崱涓嶉渶瑕佸熀鍦扮洰鏍? };
                }
            ...
            const card = player.hand.find(c => c.uid === command.payload.cardUid);
            if (!card) return { valid: false, error: '鎵嬬墝涓病鏈夎鍗＄墝' };
            if (card.type !== 'action') return { valid: false, error: '璇ュ崱鐗屼笉鏄鍔ㄥ崱' };
            const def = getCardDef(card.defId) as ActionCardDef | undefined;
            if (!def) return { valid: false, error: '鍗＄墝瀹氫箟涓嶅瓨鍦? };
            // 鐗规畩琛屽姩鍗″彧鑳藉湪鍝嶅簲绐楀彛涓墦鍑猴紝涓嶈兘鍦ㄦ甯稿嚭鐗岄樁娈典娇鐢?
