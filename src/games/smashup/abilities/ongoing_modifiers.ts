...
function registerBearCavalryModifiers(): void {
    ...
    // 规则：每个在此基地有随从的玩家 +2 爆破点，如果本回合移动过对手随从到此基地则改为 -2
    registerBreakpointModifier('bear_cavalry_youre_screwed_pod', (ctx) => {
        // 检查基地上是否有"你们已经完蛋 POD"
        const card = ctx.base.ongoingActions.find(
            a => a.defId === 'bear_cavalry_youre_screwed_pod'
        );
        if (!card) return 0;

        // 计算在此基地有随从的玩家数量
        const playersWithMinions = new Set(
            ctx.base.minions.map(m => m.controller)
        ).size;

        // 检查本回合是否移动过对手随从到此基地
        const movedThisTurn = ctx.state.movedToBasesThisTurn?.[ctx.baseIndex] ?? false;

        // 每个玩家 +2 或 -2
        const modifier = playersWithMinions * 2;

        return movedThisTurn ? -modifier : modifier;
    });
}
