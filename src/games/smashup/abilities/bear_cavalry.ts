...
import { destroyMinion, grantExtraMinion, moveMinion, getMinionPower, buildMinionTargetOptions, buildBaseTargetOptions, resolveOrPrompt, buildAbilityFeedback, createSkipOption, addTempPower, addPowerCounter, grantExtraAction } from '../domain/abilityHelpers';
...
export function registerBearCavalryInteractionHandlers(): void {
    ...
    registerInteractionHandler('bear_cavalry_cub_scout_pod_destroy', (state, playerId, value, iData, _random, timestamp) => {
        ...
        // 查找所有其他基地上的己方随从（牌面战力≤3）
        for (let i = 0; i < state.core.bases.length; i++) {
            if (i === scoutBaseIndex) continue; // 跳过幼熊斥候所在基地
