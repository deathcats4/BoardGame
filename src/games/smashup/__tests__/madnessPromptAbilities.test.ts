...
describe('米斯卡塔尼克大学 - miskatonic_it_might_just_work（它可能有用）', () => {
    it('弃2张：对当前在场己方随从各+2力量（临时）', () => {
        ...
        const played = execPlayActionWithMatch(state, '0', 'a1');
        const newState = applyEvents(state, played.events);

        // 解决交互：弃 2 张疯狂卡
        const resolve = resolveInteraction(newState, 'miskatonic_it_might_just_work', [{ cardUid: 'm1' }, { cardUid: 'm2' }], Date.now());
        const events = resolve.events;
        const finalState = applyEvents(resolve.state, events);

        const discardEvts = events.filter(e => e.type === SU_EVENTS.CARDS_DISCARDED);
        expect(discardEvts.length).toBe(1);
        expect((discardEvts[0] as any).payload.cardUids.sort()).toEqual(['m1', 'm2']);

        // 己方2个随从各获得+2临时力量
        const tempPowerEvts = events.filter(e => e.type === SU_EVENTS.TEMP_POWER_ADDED);
        expect(tempPowerEvts.length).toBe(2);
        expect(tempPowerEvts.every((e: any) => e.payload.amount === 2)).toBe(true);
        ...
        // 弃牌进入弃牌堆
        expect(finalState.players['0'].discard.some(c => c.uid === 'm1')).toBe(true);
        expect(finalState.players['0'].discard.some(c => c.uid === 'm2')).toBe(true);
    });

    it('弃0张：无效果（不产生弃牌/加力量事件）', () => {
        const state = makeStateWithMadness({
            players: {
                '0': makePlayer('0', {
                    hand: [
        ...
        const played = execPlayActionWithMatch(state, '0', 'a1');
        const newState = applyEvents(state, played.events);
        const resolve = resolveInteraction(newState, 'miskatonic_it_might_just_work', [], Date.now());
        const events = resolve.events;
        const discardEvts = events.filter(e => e.type === SU_EVENTS.CARDS_DISCARDED);
    ...
    it('有疯狂卡但无己方随从时：弃疯狂卡但无加力量事件', () => {
        ...
        const played = execPlayActionWithMatch(state, '0', 'a1');
        const newState = applyEvents(state, played.events);
        const resolve = resolveInteraction(newState, 'miskatonic_it_might_just_work', [{ cardUid: 'm1' }], Date.now());
        const events = resolve.events;
    ...
    it('状态正确（reduce 验证）', () => {
        ...
        const newState = applyEvents(state, played.events);
        const resolve = resolveInteraction(newState, 'miskatonic_it_might_just_work', [{ cardUid: 'm1' }], Date.now());
        const finalState = applyEvents(resolve.state, resolve.events);
        // 疯狂卡被弃掉（从手牌移除）
        expect(finalState.players['0'].hand.filter(c => c.defId === MADNESS_CARD_DEF_ID).length).toBe(0);
        // 随从获得+1临时力量
        expect(finalState.bases[0].minions[0].tempPowerModifier).toBe(1);
    });

    it('只影响当前在场随从：同回合后续打出的随从不获得加成', () => {
        const state = makeStateWithMadness({
            players: {
                '0': makePlayer('0', {
                    hand: [
                        makeCard('a1', 'miskatonic_it_might_just_work', 'action', '0'),
                        makeCard('m1', MADNESS_CARD_DEF_ID, 'action', '0'),
                        makeCard('minion-new', 'test_new_minion', 'minion', '0'),
                    ],
                }),
                '1': makePlayer('1'),
            },
        ...
        const played = execPlayActionWithMatch(state, '0', 'a1');
        const afterAction = applyEvents(state, played.events);
        const resolve = resolveInteraction(afterAction, 'miskatonic_it_might_just_work', [{ cardUid: 'm1' }], Date.now());
        const afterResolve = applyEvents(resolve.state, resolve.events);
        expect(afterResolve.bases[0].minions.find(m => m.uid === 'mine1')?.tempPowerModifier).toBe(1);

        const afterPlayMinion = reduce(afterResolve, {
        ...
        const newMinion = afterPlayMinion.bases[0].minions.find(m => m.uid === 'minion-new');
